...
export const useCurrentUserStore = defineStore("currentUserStore", {
    ...
    getters: {
        ...
        truncatedEmail:
            ...
            (truncateLength: number = 25) => {
                if (state.currentUser?.email.length < truncateLength)
                    return state.currentUser.email;
        ...
        role(state) {
            ...
            if (state.currentUser && state.currentUser.siteAccess) {
                const siteRole = state.currentUser.siteAccess.find(
    ...
    actions: {
        ...
        async init() {
            ...
            onAuthStateChanged($auth, async (user: User | null) => {
                ...
                if (user) {
                    ...
                    this.currentUser.siteAccess = await this.readAccess(
                        user.uid,
                    );
                }
        ...
        async readAccess(
            uid: UserProfile["uid"],
        ): Promise<UserProfile["siteAccess"]> {
            const { data } = await useFetch<UserProfile["siteAccess"]>(
                `/api/users/access`,
                {
                    params: { uid: uid },
                },
